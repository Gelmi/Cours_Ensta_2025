...
class App:
    ...
    def __init__(self, geometry, grid):
        ...
        self.canvas_cells = []
        self.colors = np.array([grid.col_dead[:-1], grid.col_life[:-1]])

    def draw(self, grid):
        surface = pg.surfarray.make_surface(self.colors[grid.cells.T])
        ...
        if (self.draw_color is not None):
            [pg.draw.line(self.screen, self.draw_color, (0, i * self.size_y), (self.width, i * self.size_y)) for i in range(grid.dimensions[0])]
            [pg.draw.line(self.screen, self.draw_color, (j * self.size_x, 0), (j * self.size_x, self.height)) for j in range(grid.dimensions[1])]
        pg.display.update()
...
if __name__ == '__main__':
    ...
    if rank == 1:
        ...
        req = comm.isend(grid, dest=0, tag=11)
        req.wait()
    ...
    while loop:
        ...
        if rank == 1:
            ...
            req.Wait()
            if send:
                comm.send(grid, dest=0, tag=11)
                send = False
        if rank == 0:
            req = comm.isend(True, dest=1, tag=12)
            grid = comm.recv(source=1, tag=11)
            t1 = time.time()
            appli.draw(grid)
            t2 = time.time()
            print(f"Temps affichage : {t2 - t1: 2.2e} secondes\n", end='')
